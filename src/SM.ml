...
let rec compile (fs, pr) =
  ...
  | Expr.Call (f, arg) -> List.concat (List.map exprC (List.rev arg))  @ [CALL (f, List.length arg, false)]
  in
    ...
    | Stmt.Call (names, arg) -> exprC (Expr.Call (names, arg))
    | Stmt.Return x -> (match x with
                    | None -> [RET false]
                    | Some x -> exprC x @ [RET true])
    in let label = nameGen#name in
    let compile' (n, (p, l, b)) =
          [LABEL n] @ [BEGIN (n, p, l)] @ helper b @ [END] in helper pr @ [END] @ List.concat (List.map compile' fs)
